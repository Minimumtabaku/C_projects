...
TEST_CASE("Copy operations", "[copy]") {
    ...
    SECTION("Range constructor only performs n copies") {
        ...
        // libc++, used with msan, sometimes uses ca instead of ~d and cc
        auto ca = t1.copy_assignments - start.copy_assignments;
        t1.copy_constructors += ca;
        t1.destructors += ca;
        t1.copy_assignments -= ca;
        REQUIRE(start == t1);
    }
